import Link from "next/link"

import { Container } from "@/components/container"
import { Footer } from "@/components/footer"
import { Header } from "@/components/header"

export default function NotFound() {
  return (
    <>
      <Header />
      <main className="min-h-[60vh] bg-[var(--color-background)] pt-[4.5rem] md:pt-20">
        <Container className="py-20 text-center">
          <p className="text-sm font-medium uppercase tracking-wider text-[var(--color-primary)]">404</p>
          <h1 className="mt-2 font-[family-name:var(--font-cardo)] text-3xl font-bold text-[var(--color-foreground)]">
            Stránka nenalezena
          </h1>
          <p className="mx-auto mt-4 max-w-md text-[var(--color-muted)]">
            Odkaz může být neplatný nebo stránka byla přesunuta.
          </p>
          <Link
            href="/"
            className="mt-8 inline-flex min-h-[48px] items-center justify-center rounded-full bg-[var(--color-primary)] px-8 font-semibold text-white hover:bg-[var(--color-primary-hover)]"
          >
            Zpět na úvod
          </Link>
        </Container>
      </main>
      <Footer />
    </>
  )
}
